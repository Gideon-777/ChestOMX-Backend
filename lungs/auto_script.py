...
def ctss_results_map(involvmenets, ctss):
    ctss_results = {
            'involvement_lul': involvmenets[0],
            'involvement_lll': involvmenets[1],
            'involvement_rul': involvmenets[2],
            'involvement_rml': involvmenets[3],
            'involvement_rll': involvmenets[4],
            'ctss_lul': ctss[0],
            'ctss_lll': ctss[1],
            'ctss_rul': ctss[2],
            'ctss_rml': ctss[3],
...
def inference(args, col, unique_id, model_name, nifti_path, output_path, stage=1):
    ...
    def callback(x, y, z, steps):
        ...
        total_num_steps = len(steps[0]) * len(steps[1]) * len(steps[2])
        percentage_steps = int((num_steps_done * 100) / total_num_steps)
        if num_steps_done % 20 == 0:
            col.document(unique_id).update({"predictionStatus": "{} prediction {}%".format(model_name, percentage_steps)})
...
def do_inference(
    ...
    try:
        ...
        for model in models:
            ...
            col.document(unique_id).update({'predictionStatus': '{} inference stage done'.format(model)})
        ...
        if 'lobes' in models and 'haa' in models:
            ...
            infiltration_path = f"{outputs_folder}/haa/{patient_id}_{unique_id}.nii.gz"
    ...
    col.document(unique_id).update({'predictionStatus': 'png files generation'})
    ...
    col.document(unique_id).update({'predictionStatus': '3D obj processing'})
    ...
    col.document(unique_id).update({'predictionStatus': 'Inference report generation'})
    ...
    col.document(unique_id).update({
        'predictionStatus': 'Finished Processing - Results ready', 
